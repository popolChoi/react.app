...
export default class Container extends Component {
  ...
  componentDidMount() {
    if (this.mainFef) {
      ...
      mainCurrent.addEventListener('scroll', (e) => {
        const scrollLocation = mainCurrent.scrollTop;
        const windowHeight = mainCurrent.clientHeight;
        const fullHeight = mainCurrent.scrollHeight;

        if (mainCurrent.scrollTop > 20) {
          if (!this.state.introduce) {
            this.setState({ introduce: true });
          }
        }

        if (mainCurrent.scrollTop > 100) {
          if (!this.state.WorkExperience) {
            this.setState({ WorkExperience: true });
          }
        }

        if (mainCurrent.scrollTop > 400) {
          if (!this.state.skill) {
  ...
  render() {
    const {
      introduce, WorkExperience, skill, education,
    } = this.state;
    return (
      <Fragment>
